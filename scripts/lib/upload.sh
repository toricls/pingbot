...
echo 'Uploading all Lambda functions into s3://'$S3_PINGBOT_RESOURCE_BUCKET'/lambda/...'
aws s3 sync $TMP_LAMBDA_ARCHIVE_DIRECTORY s3://$S3_PINGBOT_RESOURCE_BUCKET/lambda/ --delete --exclude ".*"
...
echo 'Uploading all Lambda Cognito function into s3://'$S3_PINGBOT_RESOURCE_BUCKET'/cognito/...'
aws s3 sync $TMP_LAMBDA_COGNITO_ARCHIVE_DIRECTORY s3://$S3_PINGBOT_RESOURCE_BUCKET/cognito/ --delete --exclude ".*"
